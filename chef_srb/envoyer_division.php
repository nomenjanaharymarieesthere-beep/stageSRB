...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$courrier || !in_array($courrier['statut'], ['oriente_srb', 'sortie_service'], true)) {
            throw new RuntimeException("Ce courrier ne peut pas être envoyé (statut invalide).");
        }
        if ($courrier['statut'] === 'sortie_service' && $courrier['service_cible'] !== 'SRB') {
            throw new RuntimeException("Ce courrier sortant n'est pas destiné au SRB.");
        }
        ...
        transmettre_courrier([
            ...
            'notif_message' => $estSortie
                ? "Le Chef SRB vous a transmis le courrier sortant {$courrier['reference']} (réf. départ {$courrier['reference_depart']}) pour stockage."
                : "Le Chef SRB vous a transmis le courrier {$courrier['reference']}.",
            'hist_from' => $estSortie
            ...
            'hist_to' => $estSortie
                ? "Réception du courrier sortant {$courrier['reference']} envoyé par le Chef SRB pour stockage."
                : "Réception du courrier {$courrier['reference']} envoyé par le Chef SRB.",
        ]);
...
$sortants = $db->query("SELECT * FROM courriers WHERE statut = 'sortie_service' AND service_cible = 'SRB' ORDER BY created_at ASC")->fetchAll();
