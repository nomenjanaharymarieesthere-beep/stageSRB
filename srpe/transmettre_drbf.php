...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$courrier || $courrier['service_cible'] !== 'SRPE' || $courrier['statut'] !== 'valide_chef') {
            throw new RuntimeException("Ce courrier ne peut pas être transmis actuellement.");
        }

        $pdfComp = handle_pdf_upload('pdf_complementaire', 'comp_srpe_' . $u['id']);
        ...
        transmettre_courrier([
            ...
            'from_role' => 'srpe', 'to_role' => 'drbf',
            'action' => 'transmission_drbf', 'message' => $message,
            'nouveau_statut' => 'soumis_drbf',
            'pdf_complementaire' => $pdfComp,
            'notif_message' => "Le Chef SRPE a transmis le courrier {$courrier['reference']}" . ($pdfComp && $pdfComp !== $courrier['pdf_complementaire'] ? " (avec PDF complémentaire)" : "") . " au DRBF pour validation finale.",
            'hist_from' => "Transmission du courrier {$courrier['reference']} au DRBF pour validation finale.",
            'hist_to' => "Réception du courrier {$courrier['reference']} du Chef SRPE pour validation finale.",
        ]);
    ...
    header('Location: ' . root_url('/srpe/transmettre_drbf.php'));
    exit;
}

$aValider = $db->prepare("SELECT * FROM courriers WHERE service_cible = 'SRPE' AND statut = 'valide_chef' ORDER BY created_at ASC");
$aValider->execute();
