...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$courrier || $courrier['service_cible'] !== 'SRPE') {
            throw new RuntimeException("Courrier introuvable pour le SRPE.");
        }
        if (empty($courrier['reference_depart'])) {
            throw new RuntimeException("Ce courrier n'a pas de référence de départ.");
        }

        if ($action === 'stocker' && $courrier['statut'] === 'sortie_service') {
            $db->prepare("UPDATE courriers SET statut = 'sortie_stockee', updated_at = NOW() WHERE id = ?")->execute([$courrierId]);
            add_historique($u['id'], $courrierId, 'stockage_sortie', "Stockage du courrier sortant {$courrier['reference']} (réf. départ {$courrier['reference_depart']}) au SRPE.");
            flash_set('success', "Courrier sortant {$courrier['reference']} stocké au SRPE.");
        } else {
    ...
    header('Location: ' . root_url('/srpe/courriers_sortie.php'));
    exit;
}
...
if ($search !== '') {
    $like = "%$search%";
    $stmt = $db->prepare("SELECT * FROM courriers WHERE service_cible = 'SRPE' AND statut IN ('sortie_service','sortie_stockee') AND (reference LIKE ? OR reference_depart LIKE ? OR objet LIKE ? OR expediteur LIKE ?) ORDER BY updated_at DESC");
    $stmt->execute([$like, $like, $like, $like]);
    $courriers = $stmt->fetchAll();
} else {
    $courriers = $db->query("SELECT * FROM courriers WHERE service_cible = 'SRPE' AND statut IN ('sortie_service','sortie_stockee') ORDER BY updated_at DESC")->fetchAll();
}

$pageTitle = "Courriers sortants — SRPE";
...
<div class="panel">
  ...
  <form method="get" class="search-bar">
    ...
    <?php if ($search !== ''): ?>
      <a href="<?= root_url('/srpe/courriers_sortie.php') ?>" class="clear-btn">✕ Effacer</a>
    <?php endif; ?>
  </form>
  ...
  <table>
    ...
    <tbody>
      <?php if (empty($courriers)): ?>
        <tr class="empty-row"><td colspan="7">Aucun courrier sortant pour le SRPE.</td></tr>
      <?php else: foreach ($courriers as $c): ?>
        <tr>
          ...
          <td>
            <a class="btn btn-outline btn-sm" href="<?= root_url('/srpe/fiche_imprimable.php?id=' . $c['id']) ?>" target="_blank">🖨️ Fiche</a>
          </td>
          <td>
            <?php if ($c['statut'] === 'sortie_service'): ?>
