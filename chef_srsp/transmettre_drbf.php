...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$courrier || $courrier['service_cible'] !== 'SRSP' || $courrier['statut'] !== 'valide_chef') {
            throw new RuntimeException("Ce courrier ne peut pas être transmis actuellement.");
        }

        $pdfComp = handle_pdf_upload('pdf_complementaire', 'comp_srsp_' . $u['id']);
        ...
        transmettre_courrier([
            ...
            'from_role' => 'chef_srsp', 'to_role' => 'drbf',
            'action' => 'transmission_drbf', 'message' => $message,
            'nouveau_statut' => 'soumis_drbf',
            'pdf_complementaire' => $pdfComp,
            'notif_message' => "Le Chef SRSP a transmis le courrier {$courrier['reference']}" . ($pdfComp && $pdfComp !== $courrier['pdf_complementaire'] ? " (avec PDF complémentaire)" : "") . " au DRBF pour validation finale.",
            'hist_from' => "Transmission du courrier {$courrier['reference']} au DRBF pour validation finale.",
            'hist_to' => "Réception du courrier {$courrier['reference']} du Chef SRSP pour validation finale.",
        ]);
    ...
    header('Location: ' . root_url('/chef_srsp/transmettre_drbf.php'));
    exit;
}

$aValider = $db->prepare("SELECT * FROM courriers WHERE service_cible = 'SRSP' AND statut = 'valide_chef' ORDER BY created_at ASC");
$aValider->execute();
